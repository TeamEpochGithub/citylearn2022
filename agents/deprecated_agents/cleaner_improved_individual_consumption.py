...
def calculate_next_chunk(pos_in, agent_id, time_step, remaining_battery_capacity, soc, observation):
    chunk_consumptions = []
    steps = 0
    pos = -pos_in
    ...
    if pos == -1:  # If negative consumption
        if -1 * chunk_total_consumption >= (remaining_battery_capacity - soc) / np.sqrt(
                0.83):  # If more energy can be obtained than the one necessary to charge the battery
...
def individual_consumption_policy(observation, time_step, agent_id, remaining_battery_capacity, soc, prev_consumption_sign,
                                  chunk_charge_loads, steps_in_chunk):
    # if agent_id == 1:
    #     print(observation, time_step, agent_id, remaining_battery_capacity, soc, pos_in, energies_in, steps_in)

    if time_step >= 8759:
        return 0, chunk_charge_loads, steps_in_chunk, prev_consumption_sign

    next_consumption = consumptions[agent_id][time_step]

    if next_consumption * prev_consumption_sign < 0:  # prev_consumption_sign is 1 if previous consumption was positive and -1 if it was negative
        chunk_charge_loads = calculate_next_chunk(prev_consumption_sign,
                                                  agent_id,
                                                  time_step,
    ...
    energy = -1 * prev_consumption_sign * chunk_charge_loads[steps_in_chunk]
    action = energy / remaining_battery_capacity

    return action, chunk_charge_loads, steps_in_chunk, prev_consumption_sign


class CleanerImprovedIndividualConsumptionAgent:
