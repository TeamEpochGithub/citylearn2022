...
def individual_consumption_policy(observation, time_step, agent_id, capacity, soc):
    ...
    if pos == -1: # If negative consumption
        if -1 * chunk_total_consumption >= (capacity - soc) / np.sqrt(0.83): # If more energy can be obtained than the one necessary to charge the battery
            relative_consumption = next_consumption/chunk_total_consumption
            energy = relative_consumption * (capacity - soc) / np.sqrt(0.83)
        else: # Otherwise charge with all the possible energy
            energy = -1 * next_consumption
...
class ImprovedIndividualConsumptionAgent:
    ...
    def compute_action(self, observation, agent_id):
        ...
        action_out = individual_consumption_policy(observation, collaborative_timestep, agent_id, self.remaining_battery_capacity[agent_id], self.soc[agent_id])
