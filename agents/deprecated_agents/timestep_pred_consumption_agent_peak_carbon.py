...
def positive_consumption_scenario(obs_date, chunk_consumptions, timestep, soc, agent_id):
    chunk_total_consumption = sum(chunk_consumptions)
    ...
    if chunk_total_consumption >= soc * np.sqrt(0.83):
        ...
        weight_p = 1  # Set weight of price vs emission
        weight_e = 1 - weight_p
...
def calculate_next_chunk(observation, consumption_sign, agent_id, timestep, remaining_battery_capacity, soc,
    ...
    if consumption_sign == -1:  # If negative consumption
        ...
        chunk_charge_loads = positive_consumption_scenario(date, chunk_consumptions, timestep, soc, agent_id)
