...
def get_chunk_consumptions(agent_id, timestep, consumption_sign, live_learner):
    ...
    while live_learner.predict_multiple_consumption(timestep + future_steps, True)[timestep + future_steps - 1] * consumption_sign > 0:
...
def pred_consumption_policy(observation, timestep, agent_id, remaining_battery_capacity, soc,
                            prev_consumption_sign, chunk_charge_loads, step_in_chunk, live_learner):
    if timestep >= 8759:
    ...
    if timestep < 72:
        ...
        if 6 <= hour <= 14:
            action = 0.11
        return action, chunk_charge_loads, step_in_chunk + 1, prev_consumption_sign
    ...
    def compute_action(self, observation, agent_id):
        ...
        action_out, self.chunk_charge_loads[agent_id], self.steps_in_chunk[agent_id], self.consumption_sign[agent_id] = \
            pred_consumption_policy(observation[agent_id], building_timestep, agent_id,
                                    ...
                                    self.steps_in_chunk[agent_id], self.live_learners[str(agent_id)])
